...
def get_valid_entries(address, balance):
	...
	# ignore sends
	    ...
	    # stop when we hit the current balance
            if(round(running_balance,2) >= balance):
                break
...
def get_special_entries(address, valid_entries):
    entry_count = 0
    response = wallet_api.get_transactions(address, -1)
    account = response.json()
    entries = []
...
def main():
    ...
    special_entries = get_special_entries(settings.ban_account, ticket_count)
    random.shuffle(entries)
    random.shuffle(special_entries)
    ...
    print("Bonus Draw Entries {:>3}".format(len(special_entries)))
    print("Bonus Winner: {:<30}".format(random.choice(special_entries)))
